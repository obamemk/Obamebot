...
export async function resetwarns(client, message) {
    ...
    if (message.message?.extendedTextMessage?.contextInfo?.quotedMessage) {
        ...
        return await client.sendMessage(groupId, {
            text: `馃搳 *Warns:* ${count} utilisateur(s)\n\nUsage: .resetwarns @user`
        })
    }
    ...
    if (warnStorage[warnKey]) {
        delete warnStorage[warnKey]
        await client.sendMessage(groupId, {
            text: `鉁� Warns r茅initialis茅s pour @${target.split('@')[0]}`
        })
    } else {
        await client.sendMessage(groupId, {
            text: `鈩癸笍 Aucun warn pour @${target.split('@')[0]}`
        })
    }
}
...
export async function checkwarns(client, message) {
    ...
    if (warnKeys.length === 0) {
        return await client.sendMessage(groupId, {
            text: '鉁� Aucun warn dans ce groupe.'
        })
    }
    
    let report = '馃搳 *Liste des Warns*\n\n'
...
export async function kick(client, message) {
    ...
    try {
        ...
        if (message.message?.extendedTextMessage?.contextInfo?.quotedMessage) {
            ...
            return await client.sendMessage(groupId, { text: '鉂� R茅ponds 脿 un message ou mentionne.' })
        }
        
        await client.groupParticipantsUpdate(groupId, [target], 'remove')
        await client.sendMessage(groupId, { text: `馃毇 @${target.split('@')[0]} exclu.` })
    } catch (error) {
        await client.sendMessage(groupId, { text: '鉂� Erreur' })
    }
}
...
export async function kickall(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { text: '鈿� Digital Crew - Purge...' })
        ...
        await client.sendMessage(groupId, { text: '鉁� Purge termin茅e.' })
    } catch (error) {
        await client.sendMessage(groupId, { text: '鉂� Erreur' })
    }
}
...
export async function kickall2(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { text: '鈿� Digital Crew - One Shot...' })
        await client.groupParticipantsUpdate(groupId, targets, 'remove')
        await client.sendMessage(groupId, { text: '鉁� Tous exclus.' })
    } catch (error) {
        await client.sendMessage(groupId, { text: '鉂� Erreur' })
    }
}
...
export async function promote(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { text: `馃憫 @${target.split('@')[0]} promu admin.` })
    } catch (error) {
        await client.sendMessage(groupId, { text: '鉂� Erreur' })
    }
}
...
export async function demote(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { text: `馃搲 @${target.split('@')[0]} retir茅 admin.` })
    } catch (error) {
        await client.sendMessage(groupId, { text: '鉂� Erreur' })
    }
}
...
export async function gclink(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { 
            text: `馃敆 Lien du groupe:\nhttps://chat.whatsapp.com/${code}` 
        })
    } catch (error) {
        await client.sendMessage(groupId, { text: '鉂� Impossible de g茅n茅rer le lien.' })
    }
}
